...
-- RLS (Safe policies that won't error if they exist)
DO $$ 
BEGIN
    ALTER TABLE public.conflict_sessions ENABLE ROW LEVEL SECURITY;
EXCEPTION WHEN OTHERS THEN NULL; END $$;

-- Drop generic policies if they exist to avoid confusion, or use DO blocks to create if not exists. 
-- For simplicity in Supabase SQL editor:
DROP POLICY IF EXISTS "Couple can view conflict sessions" ON public.conflict_sessions;
DROP POLICY IF EXISTS "Couple can insert conflict sessions" ON public.conflict_sessions;
DROP POLICY IF EXISTS "Couple can update conflict sessions" ON public.conflict_sessions;
